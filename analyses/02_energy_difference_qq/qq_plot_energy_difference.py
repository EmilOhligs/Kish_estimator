...
Ausfuehren:
    python analyses/qq_plot_energy_difference.py
    python analyses/qq_plot_energy_difference.py \
...
def get_delta_e(args) -> tuple[np.ndarray, str]:
    ...
    if args.energies:
        ...
        from uq_mace.predictions import find_energy_cache, get_predictions

        p = find_energy_cache(args.ensemble, args.testset)
        if p is not None:
            e_dft, e_mace = load_energies(p)
            label = p.name
        else:  # wirklich kein Cache -> MACE laeuft (einmalig)
...
def main():
    ...
    out = HERE / f"qq_energy_difference_{args.ensemble}_test{args.testset}.png"
