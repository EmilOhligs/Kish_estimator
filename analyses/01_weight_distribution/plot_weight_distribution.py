...
Ablauf:
  ...
  2. L2c-Ensemble (2 Member, wie Hilpert & Kresse 2026) ueber alle Frames laufen
     ...
     cache/predictions_<ensemble>_test<testset>.npz-Cache nutzt wie
  ...
  4. Plot (weight_distribution_*.png (neben dem Skript)) mit 4 Panels + Diagnostik:
       CV(w), Kish-N_eff, N_eff/N, Pareto-k_hat, alpha_max, Energy/Force-RMSE.

Ausfuehren (im Projekt-Root, venv aktiv):
        python analyses/plot_weight_distribution.py
        python analyses/plot_weight_distribution.py --temperature 298 --force
        python analyses/plot_weight_distribution.py --ensemble ensemble_L0c
"""
...
def get_energies(ensemble: str, testset: str, force: bool):
    ...
    Nutzt denselben cache/predictions_*-Cache wie evaluate_ensembles.py.
...
def main():
    ...
    out_png = HERE / f"weight_distribution_{args.ensemble}_test{args.testset}.png"
